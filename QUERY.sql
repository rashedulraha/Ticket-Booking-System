...
-- Query 6: Find all ticket bookings where total cost is strictly higher than the average cost of all bookings.
SELECT booking_id, match_id, total_cost 
FROM Bookings 
WHERE total_cost > (SELECT AVG(total_cost) FROM Bookings);

-- Query 7: Retrieve the top 2 most expensive matches sorted by base ticket price, skipping the absolute highest.
SELECT match_id, fixture, base_ticket_price 
FROM Matches 
